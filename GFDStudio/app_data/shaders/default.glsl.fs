...
void main()
{
    ...
    */
    ...
    reflectionColor = mix(reflectionColor, texture(uReflection, reflectionCoord.xy), hasMatFlag(cMF_HasReflectionMap));
    ...
    if (DrawMethod == 1 || DrawMethod == 2 ){ // can add proper alpha blending in the future maybe
        if (resultColor.a < dither_limit || resultColor.a < dither_alpha_clip)
                discard;
    }
